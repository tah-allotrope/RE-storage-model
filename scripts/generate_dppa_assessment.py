...
def _run_pipeline(input_path: Path, input_type: str, ppa_option: int) -> dict:
    """Run the pipeline for a single PPA option."""
    if input_type == "excel":
        return run_full_model(input_path, ppa_option=ppa_option)
    return run_model_from_json(input_path, ppa_option=ppa_option)
...
def generate_assessment(
    ...
    logger.info("Input type: %s, PPA options: %s", input_type, ppa_options)
    ...
    logger.info("Running default pipeline (PPA option 3)...")
    default_results = _run_pipeline(input_path, input_type, ppa_option=3)
    ...
    logger.info("Running all scenarios for comparison...")
    ...
    # Run sensitivity analysis (top 5 variables)
    ...
    write_cover_sheet(
        ...
        project_metadata=_build_project_metadata(default_results, input_path),
    ...
    write_assumptions_sheet(
        wb,
        assumptions_dict=_extract_assumptions(default_results),
    )
